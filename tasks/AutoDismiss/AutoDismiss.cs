...
static class AutoDismissTask {
	...
	public static bool Run(Mat frame) {
		...
		if (match == null) {
			LogThrottled($"AutoDismiss match skipped. Frame={frame.Width}x{frame.Height}, Template={template.Width}x{template.Height}");
			return false;
		}

		var point = (X: match.Value.X + template.Width / 2, Y: match.Value.Y + template.Height / 2);
		LogThrottled($"AutoDismiss match. Score={match.Value.Score:F4}, Threshold={MATCH_THRESHOLD:F2}, Frame={frame.Width}x{frame.Height}, Template={template.Width}x{template.Height}, TopLeft=({match.Value.X},{match.Value.Y}), Center=({point.X},{point.Y})");
		...
		_lastClickAt = now;
		AppLog.Write($"AutoDismiss clicked. Score={match.Value.Score:F4}, Center=({point.X},{point.Y})");
		return true;
	}
